...
export function TradeActionBar(props: {
  canExecute: boolean;
  /** Must match selected Short/Long under Futures/Spot. */
  side: TradeSide;
  /** False until the user taps Short or Long under Futures/Spot (resets when trade context changes). */
  ...
  const openShortEnabled = directionPicked && side === 'short' && canExecute;
  const openLongEnabled = directionPicked && side === 'long' && canExecute;
    ...
    >
      <div className="mx-auto max-w-lg space-y-2">
        <div className="grid grid-cols-2 gap-2">
          <button
            type="button"
            disabled={!openShortEnabled}
            title={
              !directionPicked
                ? 'Select Short under Futures / Spot first'
                : side !== 'short'
                  ? 'Switch to Short to open a short'
                  : undefined
            }
            onClick={onOpenShort}
          ...
          <button
            type="button"
            disabled={!openLongEnabled}
            title={
              !directionPicked
                ? 'Select Long under Futures / Spot first'
                : side !== 'long'
                  ? 'Switch to Long to open a long'
                  : undefined
            }
            onClick={onOpenLong}
        ...
        {!directionPicked ? (
          <p className="text-center text-[10px] leading-snug text-amber-200/85">
            Choose Short or Long under Futures / Spot, then tap the matching open button.
          </p>
        ) : null}
      </div>
      <p
